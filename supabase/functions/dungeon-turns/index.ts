  ...
  for(const c of campaigns||[]){
    ...
    if(!active){
      ...
      try{
        ...
        if(ai.text)await db.from('story_events').insert({campaign_id:c.id,actor_user_id:null,event_type:'scene_resolved',payload:{summary:ai.text,scene_turn_number:sceneTurn}});
        ...
        for(const m of members||[])await db.from('notification_outbox').insert({campaign_id:c.id,user_id:m.user_id,kind:'new_scene',title:'New AI DM Scene',body:String(summary||'The scene advanced.').slice(0,180),data:{campaignId:c.id,sceneTurn:nextTurn}});
