...
Deno.serve(async req=>{
  try{
    ...
    const {data:rows,error}=await supabase.from('notification_queue').select('*').is('delivered_at',null).order('created_at').limit(100);
    ...
    for(const n of rows||[]){
      ...
      if(res.ok){await supabase.from('notification_queue').update({delivered_at:new Date().toISOString()}).eq('id',n.id);delivered++;}
    }
    return new Response(JSON.stringify({ok:true,checked:(rows||[]).length,delivered}),{headers:{'Content-Type':'application/json'}});
