...
function inQuietHours(start:string|null,end:string|null,now=new Date()){
  ...
  const cur=now.getHours()*60+now.getMinutes(),a=mins(start),b=mins(end);
  return a===b?false:(a<b?cur>=a&&cur<b:cur>=a||cur<b);
}
...
Deno.serve(async req=>{
  ...
  if(!vapidPublic||!vapidPrivate)return json({ok:false,error:'VAPID secrets are not configured'},500);
  webpush.setVapidDetails(subject,vapidPublic,vapidPrivate);
  const admin=createClient(supabaseUrl,service);
  const {data:rows,error}=await admin.from('notification_outbox').select('*').is('sent_at',null).is('failed_at',null).lte('deliver_after',new Date().toISOString()).order('id').limit(100);
  ...
  for(const row of rows||[]){
    ...
    if(pref&&inQuietHours(pref.quiet_start,pref.quiet_end)){
      await admin.from('notification_outbox').update({deliver_after:new Date(Date.now()+30*60000).toISOString()}).eq('id',row.id);skipped++;continue;
    }
    ...
    if(!subs?.length){await admin.from('notification_outbox').update({failed_at:new Date().toISOString(),failure:'no-push-subscription'}).eq('id',row.id);failed++;continue;}
    let any=false;
    for(const sub of subs){
      try{
        await webpush.sendNotification({endpoint:sub.endpoint,keys:{p256dh:sub.p256dh,auth:sub.auth}},JSON.stringify({title:row.title,body:row.body,data:row.data||{}}));any=true;
      }catch(e:any){
        if(e?.statusCode===404||e?.statusCode===410)await admin.from('push_subscriptions').delete().eq('id',sub.id);
      }
    }
    if(any){await admin.from('notification_outbox').update({sent_at:new Date().toISOString()}).eq('id',row.id);sent++;}
    else{await admin.from('notification_outbox').update({failed_at:new Date().toISOString(),failure:'all-subscriptions-failed'}).eq('id',row.id);failed++;}
  }
