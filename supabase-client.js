...
const DB={
 ...
 story:(id,limit=60)=>q(client.from('story_events').select('*').eq('campaign_id',id).order('created_at',{ascending:false}).limit(limit)),
 ...
 knowledge:id=>q(client.from('knowledge').select('*').eq('campaign_id',id).order('created_at',{ascending:false})),quests:id=>q(client.from('quests').select('*').eq('campaign_id',id).order('updated_at',{ascending:false})),
 tokens:id=>q(client.from('tokens').select('*').eq('campaign_id',id)),saveToken:t=>q(client.from('tokens').upsert({...t,updated_at:new Date().toISOString()},{onConflict:'id'}).select().single()),
 ...
 async saveNotificationPrefs(campaign_id,patch){const u=await user();return q(client.from('notification_preferences').upsert({campaign_id,user_id:u.id,...patch,updated_at:new Date().toISOString()},{onConflict:'campaign_id,user_id'}).select().single());},
